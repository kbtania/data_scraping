import scrapy
from library_scraper.items import LibraryScraperItem

class LibrarySpider(scrapy.Spider):
    name = "library"
    allowed_domains = ['irbis-nbuv.gov.ua']
    start_urls = ['http://www.irbis-nbuv.gov.ua/cgi-bin/irbis_nbuv/cgiirbis_64.exe?S21CNR=1000&S21STN=1&S21REF=2&C21COM=S&I21DBN=UJRN&P21DBN=UJRN&S21All=%3C.%3EJJA%3DA$%3C.%3E&S21FMT=j_brief&Z21ID=&S21SRW=nz%27']
    ...
    def parse(self, response):
        ...
        for i in range(count):
            book = LibraryScraperItem()
            book["name"] = books[0].css('tr td a::text')[i].getall()[0]
            book["city"] = self.get_cities(response)[i],
            book["link"] = books[0].css('tr td a::attr(href)')[i].getall()
            yield book
